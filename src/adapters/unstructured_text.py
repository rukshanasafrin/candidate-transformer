...
class TextResumeAdapter(BaseAdapter):
    # Utilizing your system's existing skill dictionary
    KNOWN_SKILLS = [
    ...
    def extract(self, file_path: str) -> List[Dict[str, Any]]:
        text = ""
        try:
            # Reads raw plain text format (supports .txt, .md, etc.)
        ...
        # Name Extraction Strategy (Fallback safely to the first structural heading string line)
        lines = [line.strip() for line in text.split('\n') if line.strip()]
        # Discard markdown headings indicators (#) if parsing raw .md formats
        assumed_name = lines[0].lstrip('# ').split('|')[0].strip() if lines else None
        ...
        # Base Structural Record Generation
        record = {
